...
For a one-command demo that stages all three, use astra_full_demo.launch.py
instead.
...
def generate_launch_description():
    ...
    declare_model = DeclareLaunchArgument(
        'model', default_value='mistral-small-latest',
        description='Mistral model id; the free tier 429s on mistral-large-latest '
                    'after about two calls')
    ...
    declare_base_frame = DeclareLaunchArgument(
        ...
        description="astra_robot's URDF and nav2_params both use base_footprint; "
                    'get_robot_pose looks up map -> this frame')
